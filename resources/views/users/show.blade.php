    ...
    <div class="col-md-4">
        ...
        <div class="card card-primary card-outline shadow-sm border-0" style="border-radius: 15px;">
            <div class="card-body box-profile">
                ...
                <h3 class="profile-username text-center fw-bold mb-0">{{ $user->name }}</h3>
                <p class="text-muted text-center mb-4">{{ $user->employee_id }}</p>

                <div class="d-flex justify-content-around mb-4 text-center">
                    <div>
                        <h5 class="fw-bold mb-0">{{ $user->equipments->where('pivot.status', \App\Models\EquipmentUser::STATUS_BORROWING)->count() }}</h5>
                        <small class="text-muted">Đang giữ</small>
                    </div>
                    <div>
                        <h5 class="fw-bold mb-0">{{ $user->equipments->count() }}</h5>
        ...
        <div class="card shadow-sm mt-4 border-0" style="border-radius: 15px;">
            <div class="card-header border-0 bg-white pt-4">
                <h3 class="card-title fw-bold"><i class="fas fa-id-card me-2 text-primary"></i>Chi tiết liên hệ</h3>
            </div>
            <div class="card-body">
                <div class="mb-3">
                ...
                <div class="timeline mt-2">
                    @forelse($user->equipments as $equipment)
                        @php
                            ...
                            $iconColor = 'text-bg-info';
                            $statusLabel = 'Chờ duyệt';
                            if($status == \App\Models\EquipmentUser::STATUS_BORROWING) { $iconColor = 'text-bg-warning shadow-sm'; $statusLabel = 'Đang mượn'; }
                            elseif($status == \App\Models\EquipmentUser::STATUS_REJECTED) { $iconColor = 'text-bg-danger'; $statusLabel = 'Đã từ chối'; }
                            elseif($status == \App\Models\EquipmentUser::STATUS_RETURNED) { $iconColor = 'text-bg-success shadow-sm'; $statusLabel = 'Đã trả'; }
                        @endphp
                        
                        <div>
                            <i class="timeline-icon fas {{ $status == 3 ? 'fa-check' : ($status == 1 ? 'fa-laptop' : ($status == 0 ? 'fa-clock' : 'fa-times')) }} {{ $iconColor }}"></i>
                            <div class="timeline-item shadow-none border mb-4" style="border-radius: 12px; border-start: 4px solid {{ $status == 3 ? '#28a745' : ($status == 1 ? '#ffc107' : ($status == 2 ? '#dc3545' : '#17a2b8')) }} !important;">
                                <span class="time text-muted"><i class="fas fa-clock me-1"></i>{{ \Carbon\Carbon::parse($equipment->pivot->ngaymuon)->format('H:i d/m/Y') }}</span>
                                <h3 class="timeline-header fw-bold">
                                    <span class="badge {{ $iconColor }} me-2" style="font-size: 0.75rem;">{{ $statusLabel }}</span>
                                    {{ $equipment->name }}
                                </h3>
                                <div class="timeline-body">
                                    <div class="row g-3">
                                        <div class="col-sm-6">
                                            ...
                                            <div class="fw-bold text-indigo small">{{ $equipment->model }}</div>
                                        </div>
                                        <div class="col-sm-6">
                                    ...
                                    @if($equipment->pivot->description)
                                        <div class="mt-2 p-2 bg-light rounded italic small">
                                            <i class="fas fa-comment-dots me-1 opacity-50"></i>{{ $equipment->pivot->description }}
                                        </div>
                                    @endif
                                </div>
                                <div class="timeline-footer py-2">
                                    ...
                                    <a href="{{ route('equipment.show', $equipment->id) }}" class="btn btn-xs text-muted ms-2">Thông tin máy</a>
                                </div>
                            </div>
                        </div>
                    @empty
                    ...
                    @endforelse
                    <div>
                        <i class="timeline-icon fas fa-rocket text-bg-secondary"></i>
                    </div>
                </div>
            </div>
        </div>
